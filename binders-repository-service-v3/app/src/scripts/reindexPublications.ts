...
import { ElasticQuery, REPO_CONFIGS_BY_TYPE, RepositoryConfigType } from "@binders/binders-service-common/lib/elasticsearch/elasticrepository";
...
const CATEGORY = "reindex-publications"
const logger = LoggerBuilder.fromConfig(config, CATEGORY);
type IndexData = { oldIndexName: string; newIndexName: string }

const reindexPublications = async (repository: ElasticPublicationsRepository, indexData: IndexData): Promise<void> => {
...
async function validateReindexOpeation(repository: ElasticPublicationsRepository, index: string, desiredNumberOfDocuments: number): Promise<boolean> {
    repository.updateIndex(index)
    logger.info("Starting validation of reindex operation", CATEGORY)
    await repository.ensureMapping(getMapping(MappingFileName.PUBLICATION))
...
const doIt = async () => {
    ...
    const indexData = {
        oldIndexName: "publications-v3",
        newIndexName: "publications-v4",
    }
    ...
    const aliasedIndexName = REPO_CONFIGS_BY_TYPE[RepositoryConfigType.Pulbications].aliasedIndexName
    logger.info(`Starting reindexing ${aliasedIndexName} index ${indexData.oldIndexName} -> ${indexData.newIndexName}`, CATEGORY)
    await reindexPublications(repo, indexData)
