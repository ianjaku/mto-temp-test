...
export default class EmojiSuggestions extends Component<EmojiSuggestionsParams> {
    ...
    lastSelectionIsInsideWord?: any; // Immutable.Map<string, boolean>;
    ...
    onEditorStateChange = (editorState: EditorState): EditorState => {
        ...
        function decodeOffsetKey(offsetKey: string) {
    ...
    getEmojisForFilter = (): List<string> => {
        ...
        if(emojiValue.length >= 1) {
            const exactSearchValue = `:${emojiValue}:`;
            const exactSearches = [];
            const filteredValues = this.props.shortNames.filter(
                (emojiShortName) => {
                    if(emojiShortName === exactSearchValue) {
                        exactSearches.push(emojiShortName);
                        return false;
                    }
            ...
            // we show suggestions when we find exact match or length of term is longer than 1 character
            if(exactSearches.length > 0 || emojiValue.length >=2) {
                result =  List(List(exactSearches).concat(filteredValues));
                const size = result.size < 9 ? result.size : 9;
