            ...
            try {
                ...
                await client.snapshot.createRepository({
                    ...
                    body: {
                        type: repositoryType,
                        settings: {
                            bucket: repositoryOptions["bucket"],
                            region: repositoryOptions["region"],
                            access_key: repositoryOptions["accessKey"],
                            secret_key: repositoryOptions["secret"]
                        }
                    }
                });
            } catch (ex) {
...
export const createClusterSnapshot = async (clusterName: string, customSnapshotName?: string, indices?: string): Promise<void> => {
    const config = getElasticProductionConfig("binders", "5.6");
...
export const waitForSnapshot = async (client: ElasticClient, repoName: string, snapshotName: string, maxWaitInMs: number): Promise<void> => {
    ...
    while (now - start < maxWaitInMs) {
        try {
            ...
            const snapshot = snapshotResponse.snapshots[0];
