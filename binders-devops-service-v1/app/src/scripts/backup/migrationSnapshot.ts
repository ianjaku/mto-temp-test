...
const doIt = async () => {
    ...
    const snapshotName = `migration-${new Date().toISOString()}`.toLocaleLowerCase()
    const indices = "binders-binders-v3,binders-collections-v3,publications-v2,useractions-*"
    await createClusterSnapshot(options.cluster, snapshotName, indices);
};
