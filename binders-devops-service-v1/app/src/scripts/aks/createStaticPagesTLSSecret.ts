...
import {
    createTLSSecret,
    deleteSecret,
    getStaticPagesTLSSecretName
} from  "../../actions/k8s/secrets";
...
const getOptions = () => {
    ...
    const parser = new CommandLineParser("createStaticPagesTLSSecret", programDefinition);
    return parser.parse();
};
...
main( async () => {
    ...
    const secretName = getStaticPagesTLSSecretName()
    const { key, cert } = getKeyAndCert(directory);
    try {
        await deleteSecret(secretName, PRODUCTION_NAMESPACE);
    } catch (err) {
        const feedback = err.output || err.message;
        if (feedback.indexOf("(NotFound)") === -1) {
            throw err;
        }
    }
